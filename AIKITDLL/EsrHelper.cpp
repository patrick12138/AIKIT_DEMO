...
extern "C" {
	// 为WPF界面提供的语音识别结果缓冲区
	char g_speechRecognitionBuffer[4096] = { 0 };
	// 标记是否有新的识别结果
	bool g_hasNewSpeechResult = false;
	// 互斥锁保护共享数据
	CRITICAL_SECTION g_speechResultLock;
}

// 初始化互斥锁
void InitSpeechResultLock() {
	static bool initialized = false;
	if (!initialized) {
		InitializeCriticalSection(&g_speechResultLock);
		initialized = true;
	}
}

// 添加识别结果到缓冲区
void AddSpeechResult(const char* text) {
	if (!text || strlen(text) == 0) return;

	EnterCriticalSection(&g_speechResultLock);
	// 如果缓冲区已满，清空它
	if (strlen(g_speechRecognitionBuffer) > 3800) {
		memset(g_speechRecognitionBuffer, 0, sizeof(g_speechRecognitionBuffer));
	}
	// 添加新结果，使用换行分隔
	if (strlen(g_speechRecognitionBuffer) > 0) {
		strcat_s(g_speechRecognitionBuffer, sizeof(g_speechRecognitionBuffer), "\n");
	}
	strcat_s(g_speechRecognitionBuffer, sizeof(g_speechRecognitionBuffer), text);
	g_hasNewSpeechResult = true;
	LeaveCriticalSection(&g_speechResultLock);
}
...
int ESRGetRlt(AIKIT_HANDLE* handle, AIKIT_DataBuilder* dataBuilder)
{
    ...
    if (output != nullptr)
    {
        ...
        while (node != nullptr && node->value != nullptr)
        {
            ...
            // 处理readable类型结果，提取并显示w字段内容
            if (strcmp(node->key, "readable") == 0) {
                char* text = (char*)node->value;
                // 查找 "w":"??" 模式
                char* wPos = strstr(text, "\"w\":\"");
                if (wPos) {
                    wPos += 5; // 跳过 "w":"
                    char extractedText[1024] = {0};
                    int i = 0;
                    // 提取引号内的文本直到结束引号
                    while (wPos[i] != '\"' && wPos[i] != '\0' && i < 1023) {
                        extractedText[i] = wPos[i];
                        i++;
                    }
                    extractedText[i] = '\0';
                    
                    AIKITDLL::LogInfo("提取到的识别文本: %s", extractedText);
                    // 将提取到的文本添加到全局缓冲区
                    AddSpeechResult(extractedText);
                }
            }
            // 对plain类型也保留处理
            else if (strcmp(node->key, "plain") == 0) {
                AddSpeechResult((const char*)node->value);
            }
...
// 为WPF应用提供的接口函数
extern "C" __declspec(dllexport) const char* GetSpeechRecognitionText()
{
	static char buffer[4096] = { 0 };
	EnterCriticalSection(&g_speechResultLock);
	strcpy_s(buffer, sizeof(buffer), g_speechRecognitionBuffer);
	LeaveCriticalSection(&g_speechResultLock);
	return buffer;
}

extern "C" __declspec(dllexport) bool HasNewSpeechResult()
{
	bool result = false;
	EnterCriticalSection(&g_speechResultLock);
	result = g_hasNewSpeechResult;
	g_hasNewSpeechResult = false; // 重置标志
	LeaveCriticalSection(&g_speechResultLock);
	return result;
}

extern "C" __declspec(dllexport) void ClearSpeechRecognitionBuffer()
{
	EnterCriticalSection(&g_speechResultLock);
	memset(g_speechRecognitionBuffer, 0, sizeof(g_speechRecognitionBuffer));
	g_hasNewSpeechResult = false;
	LeaveCriticalSection(&g_speechResultLock);
}